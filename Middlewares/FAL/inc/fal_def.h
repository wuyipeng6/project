...
#define FAL_SW_VERSION      "0.4.99"


//在FAL_PART_HAS_TABLE_CFG没有定义的情况下，会用到这四个内存管理宏，否则用不到
//我们的工程有自己的mymalloc，但是调用接口的参数与c库的malloc不同，所以不能用，
//所以只能1、：FAL_PART_HAS_TABLE_CFG需要定义，或者2、：修改我们的栈内存大小不为0，使用标准c库的malloc接口
#ifndef FAL_MALLOC
