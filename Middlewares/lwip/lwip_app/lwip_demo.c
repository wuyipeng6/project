...
uint16_t g_temp = 0;  /* 温度值 */
uint16_t g_humid = 0; /* 湿度值 */
...
/**
 * @brief       规范化payload字符串，兼容中文输入法导致的智能引号/全角符号
 * @param       src: 原始字符串
 * @param       dst: 目标缓冲区
 * @param       dst_size: 目标缓冲区大小
 * @retval      1: 发生过替换 0: 未替换
 */
...
/**
 * @brief       统一解析payload，提取item类型和值
 * @param       payload: MQTT负载
 * @param       item_type: 输出item类型
 * @param       item_value: 输出item数值
 * @retval      0: 成功 -1: 失败
 */
...
/**
 * @brief       统一解析下行payload，根据item类型执行不同操作
 * @param       topic: MQTT主题
 * @param       payload: 负载字符串（JSON格式）
 * @note        当前支持item: SG90、UPdata_info
 */
